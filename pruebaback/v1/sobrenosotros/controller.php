...
class Controlador
{
    ...
    public function getUltimoId(){
        $con = new Conexion();
        $sql = "SELECT id FROM sobre_nosotros ORDER BY id DESC LIMIT 1;";
        $rs = mysqli_query($con->getConnection(), $sql);
        if ($rs) {
            while ($tupla = mysqli_fetch_assoc($rs)) {
                array_push($this->lista, $tupla);
    ...
    public function postNew($_new)
    {
        ...
        $sql = "INSERT INTO sobre_nosotros (id, logo_color,descripcion, activo) VALUES ($id,'$_new->logo_color','$_new->descripcion',$_new->activo)";
        // echo $sql;
        $rs = [];
        try {
            $rs = mysqli_query($con->getConnection(), $sql);
    ...
    public function patchEncenderApagar($_id, $_action)
    {
        $con = new Conexion();
        // Obtener los valores del objeto $_new
        $sql = "UPDATE sobre_nosotros SET activo = $_action WHERE id = $_id";
        // echo $sql;
        $rs = [];
        try {
            $rs = mysqli_query($con->getConnection(), $sql);
    ...
    public function updateRegistroById($_id,$logo_color,$descripcion)
    {
        $con = new Conexion();
        $cantidadSet=0;
        $setFinal="";
        if(isset($logo_color)){
            $setFinal.=" logo_color='$logo_color'";
            $cantidadSet++;
        }
        ...
        $sql = "UPDATE sobre_nosotros SET $setFinal WHERE id = $_id";
        // echo $sql;
        $rs = [];
        try {
            $rs = mysqli_query($con->getConnection(), $sql);
    ...
    public function deleteById($_id)
    {
        ...
        try {
            ...
            $existsSql = "SELECT COUNT(*) as count FROM sobre_nosotros WHERE id = $_id";
