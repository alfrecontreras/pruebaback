...
class Controlador
{
    ...
    public function postNew($_new)
    {
        ...
        $sql = "INSERT INTO mantenedor (id, nombre, activo) VALUES ($id, '$_new->nombre', $_new->activo)";
        // echo $sql;
        $rs = [];
        try {
            $rs = mysqli_query($con->getConnection(), $sql);
    ...
    public function patchEncenderApagar($_id, $_action)
    {
        $con = new Conexion();
        // Obtener los valores del objeto $_new
        $sql = "UPDATE mantenedor SET activo = $_action WHERE id = $_id";
        // echo $sql;
        $rs = [];
        try {
            $rs = mysqli_query($con->getConnection(), $sql);
    ...
    public function deleteById($_id)
    {
        ...
        try {
            ...
            $existsSql = "SELECT COUNT(*) as count FROM mantenedor WHERE id = $_id";
